      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-4 sm:gap-6">
        ...
        <div className="space-y-4">
          <RecommendationsPanel 
            compact={true}
            context={{
              machineStatus: enrichedMachines.find(m => m.status === 'running') ? 'running' : 'stopped',
              currentProduct: orders.find(o => o.status === 'running')?.product_name,
              recentProblems: [],
              operatingConditions: {
                runningMachines: enrichedMachines.filter(m => m.status === 'running').length,
                totalMachines: enrichedMachines.length,
                averageEfficiency: enrichedMachines.reduce((acc, m) => acc + m.efficiency, 0) / enrichedMachines.length || 0
              }
            }}
          />
        </div>
        ...
        {/* Tonnage by Shift */}
        <div className="space-y-4">
          <TonnageByShiftCard className="border-l-4 border-l-purple-500" />
        </div>
      </div>
    </div>
  );
